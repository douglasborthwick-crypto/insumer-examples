...
|---|---|---|---|---|
| `wallet_state` | InsumerAPI | `wallet_state` | **Direct** | Foundation layer. 37 chains (31 EVM + Solana + XRPL + Bitcoin + Tron + Stellar + Sui). Per-condition `blockNumber`, `blockTimestamp`, `conditionHash`. `sub` claim (JWT) or `wallet` field (`/v1/trust`) is wallet-bound inside signature scope. ES256 P1363 or compact JWS. |
